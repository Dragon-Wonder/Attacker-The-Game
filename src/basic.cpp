...
string EndOfEffectString(string Target, unsigned char Effect) {
	string TempStr = "";
...
string StartOfEffectString(string Target, unsigned char Effect) {
	string TempStr = "";
	//Returns string describing status effect ending.
	if (Target == "player") {
		switch (Effect) {
...
void ShowOpeningMessage() {for (unsigned char i = 0; i < 16; i++){printf("%s",OpeningMessage[i].c_str());}}

void ShowWinningMessage() {for (unsigned char i = 0; i < 6; i++) {printf("%s",WinningMessage[i].c_str());}}
